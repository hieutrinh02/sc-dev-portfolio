# Smart Contract Dev Portfolio
Smart contract developer portfolio by [@hieutrinh02](https://github.com/hieutrinh02).

This repository showcases the blockchain projects I've built as part of my development as a smart contract developer.

---

## <img src="./assets/logos/ethereum.svg" width="10" style="margin-right:3px;"/> EVM Projects

| Category | Project | Repository | Status |
|---------|--------|------------|---------|
| NFT | NFT Marketplace | evm-nft-marketplace | In Progress |
| DeFi | AMM V2 | evm-amm-v2 | In Progress |
| DeFi | AMM V3 | evm-amm-v3 | In Progress |
| DeFi | Lending/Borrowing | evm-lending | In Progress |

## <img src="./assets/logos/starknet.svg" width="16" style="margin-right:3px;"/> Starknet Projects

| Category | Project | Repository | Status |
|:---------|:--------|:------------|:---------|
| NFT | NFT Marketplace | starknet-nft-marketplace | In Progress |
| DeFi | AMM V2 | starknet-amm-v2 | In Progress |
| DeFi | AMM V3 | starknet-amm-v3 | In Progress |
| DeFi | Lending/Borrowing | starknet-lending | In Progress |

## <img src="./assets/logos/solana.svg" width="18" style="margin-right:3px;"/> Solana Projects

| Category | Project | Repository | Status |
|:---------|:--------|:------------|:---------|
| NFT | NFT Marketplace | solana-nft-marketplace | Planned |
| DeFi | AMM V2 | solana-amm-v2 | Planned |
| DeFi | AMM V3 | solana-amm-v3 | Planned |
| DeFi | Lending/Borrowing | solana-lending | Planned |
